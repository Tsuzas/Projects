            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    for (int j = 5; j > -1 ; j--)
                    {
                        ...
                        string[] results = dogCharacteristic.Split(',');
                        
                        foreach (string s in results)
                        {
                            foreach (string icon in searchingIcons)
                            {
                                Console.Write($"\rsearching our dog {ourAnimals[i, 3]} for {s} {icon}");
                                Thread.Sleep(250);
                            }
                    ...
                    string[] result = dogCharacteristic.Split(',');
                    Array.Sort(result);
                    foreach (string s in result)
                    {
                        if (dogDescription.Contains(s))
                        {
                            // #3b update message to reflect term 
                            // #3c set a flag "this dog" is a match
                            Console.WriteLine($"Our dog {ourAnimals[i, 3]} is a {s} match!");
                            noMatchesDog = false;
                        }
                    }

                    Console.WriteLine($"Pet Searched :");
